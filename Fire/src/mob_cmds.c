/*
 * RAM $Id: mob_cmds.c 67 2009-01-05 00:39:32Z quixadhal $
 */
...
void mob_interpret( CHAR_DATA *ch, const char *argument )
{
    ...
    log_error( "Mob_interpret: invalid cmd from mob %d: '%s'",
...
void do_mpstat( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
        ch_printf( ch, "Mpstat whom?\r\n" );
        return;
    }

    if ( ( victim = get_char_world( ch, arg ) ) == NULL )
    {
        ch_printf( ch, "No such creature.\r\n" );
        return;
    }

    if ( !IS_NPC( victim ) )
    {
        ch_printf( ch, "That is not a mobile.\r\n" );
        return;
    }

    if ( ( victim = get_char_world( ch, arg ) ) == NULL )
    {
        ch_printf( ch, "No such creature visible.\r\n" );
        return;
    }
    ...
    {
        ch_printf( ch, "[No programs set]\r\n" );
        return;
    }

    for ( i = 0, mprg = victim->pIndexData->mprogs; mprg != NULL; mprg = mprg->next )

    {
        ch_printf( ch, "[%2d] Trigger [%-8s] Program [%4d] Phrase [%s]\r\n",
                   ++i,
                   mprog_type_to_name( mprg->trig_type ), mprg->vnum, mprg->trig_phrase );
    }
...
void do_mpgecho( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( argument[0] == '\0' )
    {
        log_error( "MpGEcho: missing argument from vnum %d",
    ...
    for ( d = descriptor_list; d; d = d->next )
    {
        if ( d->connected == CON_PLAYING )
        {
            if ( IS_IMMORTAL( d->character ) )
                ch_printf( d->character, "Mob echo> " );
            ch_printf( d->character, "%s", argument );
...
void do_mpzecho( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( argument[0] == '\0' )
    {
        log_error( "MpZEcho: missing argument from vnum %d",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
...
void do_mpkill( CHAR_DATA *ch, const char *argument )
{
    ...
    {
        log_error( "MpKill: Charmed mob attacking master from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
...
void do_mpmload( CHAR_DATA *ch, const char *argument )
{
    ...
    {
        log_error( "Mpmload: bad mob index (%d) from mob %d",
...
void do_mpoload( CHAR_DATA *ch, const char *argument )
{
    ...
    {
        log_error( "Mpoload: Bad syntax from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
    ...
    else
    {
        ...
        {
            log_error( "Mpoload: Bad level from vnum %d.",
                       IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
            return;
        }
    }
    ...
    {
        log_error( "Mpoload: Bad vnum arg from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
...
void do_mppurge( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( ( victim = get_char_room( ch, arg ) ) == NULL )
    {
        ...
        {
            log_error( "Mppurge: Bad argument from vnum %d.",
                       IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        }
        return;
    }
    ...
    if ( !IS_NPC( victim ) )
    {
        log_error( "Mppurge: Purging a PC from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
    extract_char( victim, true );
...
void do_mpgoto( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
        log_error( "Mpgoto: No argument from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
    ...
    {
        log_error( "Mpgoto: No such location from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
...
void do_mpat( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( arg[0] == '\0' || argument[0] == '\0' )
    {
        log_error( "Mpat: Bad argument from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
    ...
    if ( ( location = find_location( ch, arg ) ) == NULL )
    {
        log_error( "Mpat: No such location from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
...
void do_mptransfer( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( arg1[0] == '\0' )
    {
        log_error( "Mptransfer: Bad syntax from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
    ...
    else
    {
        ...
        {
            log_error( "Mptransfer: No such location from vnum %d.",
                       IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
            return;
        }
...
void do_mpgtransfer( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( arg1[0] == '\0' )
    {
        log_error( "Mpgtransfer: Bad syntax from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
...
void do_mpforce( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( arg[0] == '\0' || argument[0] == '\0' )
    {
        log_error( "Mpforce: Bad syntax from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
...
void do_mpgforce( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( arg[0] == '\0' || argument[0] == '\0' )
    {
        log_error( "MpGforce: Bad syntax from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
...
void do_mpvforce( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( arg[0] == '\0' || argument[0] == '\0' )
    {
        log_error( "MpVforce: Bad syntax from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
    ...
    if ( !is_number( arg ) )
    {
        log_error( "MpVforce: Non-number argument vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
...
void do_mpcast( CHAR_DATA *ch, const char *argument )
{
    ...
    char                    target[MAX_INPUT_LENGTH] = "\0\0\0\0\0\0\0";
    CHAR_DATA              *vch = NULL;
    OBJ_DATA               *obj = NULL;
    void                   *victim = NULL;
    int                     sn = 0;
    ...
    if ( spell[0] == '\0' )
    {
        log_error( "MpCast: Bad syntax from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
    ...
    {
        log_error( "MpCast: No such spell from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
...
void do_mpdamage( CHAR_DATA *ch, const char *argument )
{
    ...
    CHAR_DATA              *victim_next = NULL;
    char                    target[MAX_INPUT_LENGTH] = "\0\0\0\0\0\0\0";
    char                    min[MAX_INPUT_LENGTH] = "\0\0\0\0\0\0\0";
    char                    max[MAX_INPUT_LENGTH] = "\0\0\0\0\0\0\0";
    int                     low = 0;
    int                     high = 0;
    bool                    fAll = false;
    bool                    fKill = false;
    ...
    if ( target[0] == '\0' )
    {
        log_error( "MpDamage: Bad syntax from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
    ...
    {
        log_error( "MpDamage: Bad damage min vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
    if ( is_number( max ) )
    ...
    {
        log_error( "MpDamage: Bad damage max vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
    one_argument( argument, target );
...
void do_mpremember( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( arg[0] != '\0' )
        ...
        log_error( "MpRemember: missing argument from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
}
...
void do_mpdelay( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( !is_number( arg ) )
    {
        log_error( "MpDelay: invalid arg from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
...
void do_mpcall( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
        log_error( "MpCall: missing arguments from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
    if ( ( prg = get_mprog_index( atoi( arg ) ) ) == NULL )
    {
        log_error( "MpCall: invalid prog from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
    vch = NULL;
...
void do_mpflee( CHAR_DATA *ch, const char *argument )
{
    ...
    for ( attempt = 0; attempt < MAX_EXIT; attempt++ )
    {
        door = number_door(  );
...
void do_mpotransfer( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
        log_error( "MpOTransfer: Missing argument from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
    one_argument( argument, buf );
    ...
    {
        log_error( "MpOTransfer: No such location from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
...
void do_mpremove( CHAR_DATA *ch, const char *argument )
{
    ...
    {
        log_error( "MpRemove: Invalid object from vnum %d.",
                   IS_NPC( ch ) ? ch->pIndexData->vnum : 0 );
        return;
    }
    else
