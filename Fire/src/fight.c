/*
 * RAM $Id: fight.c 69 2009-01-11 18:13:26Z quixadhal $
 */
    ...
    for ( rch = ch->in_room->people; rch != NULL; rch = rch_next )
    {
        ...
        if ( IS_AWAKE( rch ) && rch->fighting == NULL )
        {
            ...
            if ( !IS_NPC( ch ) && IS_NPC( rch )
                 ...
                 && rch->level + 6 > victim->level && can_see( rch, ch ) )
            {
...
void multi_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
    ...
    if ( ( sa = skill_lookup( "second attack" ) ) == -1 )
    {
        log_error( "Can't find the \"%s\" skill in %s?", "second attack", __FUNCTION__ );
        return;
    }

    if ( ( ta = skill_lookup( "third attack" ) ) == -1 )
    {
        log_error( "Can't find the \"%s\" skill in %s?", "third attack", __FUNCTION__ );
        return;
    }
    ...
    {
        one_hit( ch, victim, dt );
        check_improve( ch, sa, true, 5 );
        if ( ch->fighting != victim )
            return;
    }

    chance = get_skill( ch, ta ) / 4;
...
void one_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
    ...
    /*
     * Calculate to-hit-armor-iclass-0 versus armor.
     */
    if ( IS_NPC( ch ) )
    {
        thac0_00 = 20;
     ...
     */
    {
        ...
        if ( ( ed = skill_lookup( "enhanced damage" ) ) == -1 )
        {
            log_error( "Can't find the \"%s\" skill in %s?", "enhanced damage",
                       __FUNCTION__ );
            return;
        }
        if ( get_skill( ch, ed ) > 0 )
        {
            diceroll = number_percent(  );
            if ( diceroll <= get_skill( ch, ed ) )
            {
                check_improve( ch, ed, true, 6 );
    ...
    if ( result && wield != NULL )
    {
        if ( ch->fighting == victim && IS_WEAPON_STAT( wield, WEAPON_POISON ) )
        {
            ...
            if ( ( ps = skill_lookup( "poison" ) ) == -1 )
            {
                log_error( "Can't find the \"%s\" skill in %s?", "poison", __FUNCTION__ );
                return;
            }
            if ( ( poison = affect_find( wield->affected, ps ) ) == NULL )
                level = wield->level;
            else
                level = poison->level;
...
bool damage( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt, int dam_type, bool show )
{
    ...
    {
        log_balance( ch, "%d damage: more than 1200 points!", dam );
        dam = 1200;
        if ( !IS_IMMORTAL( ch ) )
        {
            OBJ_DATA               *obj;
            ...
            ch_printf( ch, "You really shouldn't cheat.\r\n" );
            if ( obj != NULL )
                extract_obj( obj );
        }
    ...
    switch ( victim->position )
    {
        ...
        case POS_STUNNED:
            act( "$n is stunned, but will probably recover.",
                 victim, NULL, NULL, TO_ROOM );
            ch_printf( victim, "You are stunned, but will probably recover.\r\n" );
            break;

        case POS_DEAD:
            act( "$n is DEAD!!", victim, 0, 0, TO_ROOM );
            ch_printf( victim, "You have been KILLED!!\r\n\r\n" );
            break;

        default:
            if ( dam > victim->max_hit / 4 )
                ch_printf( victim, "That really did HURT!\r\n" );
            if ( victim->hit < victim->max_hit / 4 )
                ch_printf( victim, "You sure are BLEEDING!\r\n" );
            break;
    }
    ...
    if ( victim->position == POS_DEAD )
    {
        ...
        log_kill( ch, victim, "%s killed by %s at %s [#%d]",
                  NAME( victim ), NAME( ch ), ch->in_room->name, ch->in_room->vnum );

        wiz_printf( NULL, NULL, IS_NPC( victim ) ? WIZ_MOBDEATHS : WIZ_DEATHS, 0, 0,
...
bool is_safe( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( IS_NPC( victim ) )
    {
        ...
        {
            ch_printf( ch, "Not in this room.\r\n" );
            return true;
        }

        if ( victim->pIndexData->pShop != NULL )
        {
            ch_printf( ch, "The shopkeeper wouldn't like that.\r\n" );
            return true;
        }
        ...
        {
            ch_printf( ch, "I don't think Mota would approve.\r\n" );
            return true;
        }
        ...
        if ( !IS_NPC( ch ) )
        {
            ...
            {
                ch_printf( ch, "You don't own that monster.\r\n" );
                return true;
            }
        }
    }
    ...
    else
    {
        ...
        if ( IS_NPC( ch ) )
        {
            ...
            {
                ch_printf( ch, "Players are your friends!\r\n" );
                return true;
            }
        }
        /*
        ...
        else
        {
            if ( !is_clan( ch ) )
            {
                ch_printf( ch, "Join a clan if you want to kill players.\r\n" );
                return true;
            }
            ...
            if ( !is_clan( victim ) )
            {
                ch_printf( ch, "They aren't in a clan, leave them alone.\r\n" );
                return true;
            }

            if ( ch->level > victim->level + 8 )
            {
                ch_printf( ch, "Pick on someone your own size.\r\n" );
                return true;
            }
        }
    }
...
void check_killer( CHAR_DATA *ch, CHAR_DATA *victim )
{
     ...
     */
    if ( IS_NPC( victim )
         || IS_SET( victim->act, PLR_KILLER ) || IS_SET( victim->act, PLR_THIEF ) )
        return;
    ...
    if ( IS_SET( ch->affected_by, AFF_CHARM ) )
    {
        if ( ch->master == NULL )
        {
            log_balance( ch, "AFF_CHARM set on %s, but no master is set?", NAME( ch ) );
            affect_strip( ch, skill_lookup( "charm person" ) );
            REMOVE_BIT( ch->affected_by, AFF_CHARM );
            return;
        }
/*
        ch_printf( ch->master, "*** You are now a KILLER!! ***\r\n" );
...
*/
    ...
    ch_printf( ch, "*** You are now a KILLER!! ***\r\n" );
    SET_BIT( ch->act, PLR_KILLER );
    wiz_printf( ch, NULL, WIZ_FLAGS, 0, 0,
                "$N is attempting to murder %s", NAME( victim ) );
    save_char_obj( ch );
    return;
}
...
bool check_parry( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( ( sn = skill_lookup( "parry" ) ) == -1 )
    {
        log_error( "Can't find the \"%s\" skill in %s?", "parry", __FUNCTION__ );
        return false;
    }

    chance = get_skill( victim, sn ) / 2;
...
bool check_shield_block( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( ( sn = skill_lookup( "shield block" ) ) == -1 )
    {
        log_error( "Can't find the \"%s\" skill in %s?", "shield block", __FUNCTION__ );
        return false;
    }

    chance = get_skill( victim, sn ) / 5 + 3;
...
bool check_dodge( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( ( sn = skill_lookup( "dodge" ) ) == -1 )
    {
        log_error( "Can't find the \"%s\" skill in %s?", "dodge", __FUNCTION__ );
        return false;
    }
...
void set_fighting( CHAR_DATA *ch, CHAR_DATA *victim )
{
    if ( ch->fighting != NULL )
    {
        log_error( "%s already fighting in call to set_fighting.", NAME( ch ) );
        return;
    }
...
void group_gain( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( members == 0 )
    {
        log_error( "Group with %d members", members );
        members = 1;
    ...
    for ( lch = ch->in_room->people; lch != NULL; lch = lch->next_in_room )
    {
        ...
        if ( lch->level > highest_level )
            highest_level = lch->level;
    }
...
/*        Taken out, add it back if you want it
        if ( gch->level - highest_level >= 5 )
        {
            ch_printf( gch, "You are too high for this group.\r\n" );
            continue;
        }

        if ( gch->level - highest_level <= -5 )
        {
            ch_printf( gch, "You are too low for this group.\r\n" );
            continue;
        }
*/
        ...
        for ( obj = gch->carrying; obj != NULL; obj = obj_next )
        {
                 ...
                 || ( IS_OBJ_STAT( obj, ITEM_ANTI_NEUTRAL ) && IS_NEUTRAL( gch ) ) )
            {
                ...
                else
                {
                    ...
                    {
                        act( "$p dissolves into smoke.", gch, obj, NULL, TO_ROOM );
                        act( "$p dissolves into smoke.", gch, obj, NULL, TO_CHAR );
                        extract_obj( obj );
                    }
                }
            }
...
int xp_compute( CHAR_DATA *gch, CHAR_DATA *victim, int total_levels )
{
    ...
    else if ( align > 500 )                            /* monster is more good than
                                                        * slayer */
    {
    ...
    else if ( align < -500 )                           /* monster is more evil than
                                                        * slayer */
    {
...
{
    char                    buf1[256] = "\0\0\0\0\0\0\0";
    char                    buf2[256] = "\0\0\0\0\0\0\0";
    char                    buf3[256] = "\0\0\0\0\0\0\0";
    const char             *vs = NULL;
    const char             *vp = NULL;
    ...
    else
    {
        ...
        {
            log_error( "Bad damage type %d", dt );
            dt = TYPE_HIT;
...
void do_berserk( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( ( sn = skill_lookup( "berserk" ) ) == -1 )
    {
        log_error( "Can't find the \"%s\" skill in %s?", "berserk", __FUNCTION__ );
        return;
    }
    ...
    {
        ch_printf( ch, "You turn red in the face, but nothing happens.\r\n" );
        return;
    }

    if ( IS_AFFECTED( ch, AFF_BERSERK ) || is_affected( ch, sn )
         || is_affected( ch, skill_lookup( "frenzy" ) ) )
    {
        ch_printf( ch, "You get a little madder.\r\n" );
        return;
    }

    if ( IS_AFFECTED( ch, AFF_CALM ) )
    {
        ch_printf( ch, "You're feeling to mellow to berserk.\r\n" );
        return;
    }

    if ( ch->mana < 50 )
    {
        ch_printf( ch, "You can't get up enough energy.\r\n" );
        return;
    }
    ...
    if ( number_percent(  ) < chance )
    {
        ...
        ch_printf( ch, "Your pulse races as you are consumed by rage!\r\n" );
    ...
    else
    {
        ...
        ch_printf( ch, "Your pulse speeds up, but nothing happens.\r\n" );
...
void do_bash( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( ( sn = skill_lookup( "bash" ) ) == -1 )
    {
        log_error( "Can't find the \"%s\" skill in %s?", "bash", __FUNCTION__ );
        return;
    }
    if ( ( chance = get_skill( ch, sn ) ) == 0
         ...
         || ( !IS_NPC( ch ) && ch->level < skill_table[sn].skill_level[ch->iclass] ) )
    {
        ch_printf( ch, "Bashing? What's that?\r\n" );
        return;
    }
    ...
    if ( arg[0] == '\0' )
    {
        ...
        if ( victim == NULL )
        {
            ch_printf( ch, "But you aren't fighting anyone!\r\n" );
            return;
        }
    }
    ...
    if ( victim == ch )
    {
        ch_printf( ch, "You try to bash your brains out, but fail.\r\n" );
        return;
    }
    ...
    if ( !IS_NPC( victim ) && chance < get_skill( victim, skill_lookup( "dodge" ) ) )
    {
/*
 ...
 */
    ...
    if ( number_percent(  ) < chance )
    {
        ...
        WAIT_STATE( ch, skill_table[sn].beats );
        victim->position = POS_RESTING;
        damage( ch, victim, number_range( 2, 2 + 2 * ch->size + chance / 20 ), sn,
                DAM_BASH, false );
    ...
    else
    {
        ...
        WAIT_STATE( ch, skill_table[sn].beats * 3 / 2 );
    }
    check_killer( ch, victim );
}
...
void do_dirt( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( ( sn = skill_lookup( "dirt" ) ) == -1 )
    {
        log_error( "Can't find the \"%s\" skill in %s?", "dirt", __FUNCTION__ );
        return;
    }
    if ( ( chance = get_skill( ch, sn ) ) == 0
         ...
         || ( !IS_NPC( ch ) && ch->level < skill_table[sn].skill_level[ch->iclass] ) )
    {
        ch_printf( ch, "You get your feet dirty.\r\n" );
        return;
    }
    ...
    if ( arg[0] == '\0' )
    {
        ...
        if ( victim == NULL )
        {
            ch_printf( ch, "But you aren't in combat!\r\n" );
            return;
        }
    }
    ...
    if ( chance == 0 )
    {
        ch_printf( ch, "There isn't any dirt to kick.\r\n" );
        return;
    }
    ...
    if ( number_percent(  ) < chance )
    {
        ...
        damage( ch, victim, number_range( 2, 5 ), sn, DAM_NONE, false );
        ch_printf( victim, "You can't see a thing!\r\n" );
        check_improve( ch, sn, true, 2 );
        WAIT_STATE( ch, skill_table[sn].beats );
...
void do_trip( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( ( sn = skill_lookup( "trip" ) ) == -1 )
    {
        log_error( "Can't find the \"%s\" skill in %s?", "trip", __FUNCTION__ );
        return;
    }
    if ( ( chance = get_skill( ch, sn ) ) == 0
         ...
         || ( !IS_NPC( ch ) && ch->level < skill_table[sn].skill_level[ch->iclass] ) )
    {
        ch_printf( ch, "Tripping?  What's that?\r\n" );
        return;
    }
    ...
    if ( victim == ch )
    {
        ch_printf( ch, "You fall flat on your face!\r\n" );
    ...
    if ( number_percent(  ) < chance )
    {
        ...
        WAIT_STATE( ch, skill_table[sn].beats );
        victim->position = POS_RESTING;
        damage( ch, victim, number_range( 2, 2 + 2 * victim->size ), sn, DAM_BASH, true );
    }
    else
    {
        damage( ch, victim, 0, sn, DAM_BASH, true );
        WAIT_STATE( ch, skill_table[sn].beats * 2 / 3 );
        check_improve( ch, sn, false, 1 );
    }
    check_killer( ch, victim );
}
...
/*  Allow player killing
    if ( !IS_NPC(victim) )
    {
        ...
        {
            ch_printf( ch, "You must MURDER a player.\r\n" );
            return;
        }
    }
*/
    if ( victim == ch )
    {
        ch_printf( ch, "You hit yourself.  Ouch!\r\n" );
    ...
    if ( ch->position == POS_FIGHTING )
    {
        ch_printf( ch, "You do the best you can!\r\n" );
        return;
    }
...
{
    ch_printf( ch, "If you want to MURDER, spell it out.\r\n" );
    return;
}
...
void do_murder( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( victim == ch )
    {
        ch_printf( ch, "Suicide is a mortal sin.\r\n" );
        return;
    }
...
void do_backstab( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( ( sn = skill_lookup( "backstab" ) ) == -1 )
    {
        log_error( "Can't find the \"%s\" skill in %s?", "backstab", __FUNCTION__ );
        return;
    }
    if ( arg[0] == '\0' )
    {
        ch_printf( ch, "Backstab whom?\r\n" );
        return;
    }

    if ( ch->fighting != NULL )
    {
        ch_printf( ch, "You're facing the wrong end.\r\n" );
        return;
    }
    ...
    if ( victim == ch )
    {
        ch_printf( ch, "How can you sneak up on yourself?\r\n" );
        return;
    }
    ...
    {
        ch_printf( ch, "You need to wield a weapon to backstab.\r\n" );
        return;
    }
    ...
    if ( number_percent(  ) < get_skill( ch, sn )
         || ( get_skill( ch, sn ) >= 2 && !IS_AWAKE( victim ) ) )
    {
        check_improve( ch, sn, true, 1 );
...
void do_flee( CHAR_DATA *ch, const char *argument )
{
    ...
    for ( attempt = 0; attempt < 6; attempt++ )
    {
        ...
        if ( !IS_NPC( ch ) )
        {
            ch_printf( ch, "You flee from combat!\r\n" );
            if ( ( ch->iclass == 2 ) && ( number_percent(  ) < 3 * ( ch->level / 2 ) ) )
                ch_printf( ch, "You snuck away safely.\r\n" );
            else
            {
                ch_printf( ch, "You lost 10 exp.\r\n" );
                gain_exp( ch, -10 );
            }
        }
    ...
    ch_printf( ch, "PANIC! You couldn't escape!\r\n" );
    return;
}
...
void do_rescue( CHAR_DATA *ch, const char *argument )
{
    ...
    one_argument( argument, arg );
    if ( ( sn = skill_lookup( "rescue" ) ) == -1 )
    {
        log_error( "Can't find the \"%s\" skill in %s?", "rescue", __FUNCTION__ );
        return;
    }
    if ( arg[0] == '\0' )
    {
        ch_printf( ch, "Rescue whom?\r\n" );
        return;
    }
    ...
    if ( victim == ch )
    {
        ch_printf( ch, "What about fleeing instead?\r\n" );
        return;
    }
    ...
    if ( ch->fighting == victim )
    {
        ch_printf( ch, "Too late.\r\n" );
        return;
    }

    if ( ( fch = victim->fighting ) == NULL )
    {
        ch_printf( ch, "That person is not fighting right now.\r\n" );
        return;
    }
    ...
    if ( ch->fighting == victim->fighting && fch->fighting == ch )
    {
        ch_printf( ch, "You are already fighting that target!\r\n" );
        return;
    }
    ...
    if ( number_percent(  ) > get_skill( ch, sn ) )
    {
        ch_printf( ch, "You fail the rescue.\r\n" );
        check_improve( ch, sn, false, 1 );
        return;
    }
...
void do_kick( CHAR_DATA *ch, const char *argument )
{
    ...
    if ( ( sn = skill_lookup( "kick" ) ) == -1 )
    {
        log_error( "Can't find the \"%s\" skill in %s?", "kick", __FUNCTION__ );
        return;
    }
    if ( !IS_NPC( ch ) && ch->level < skill_table[sn].skill_level[ch->iclass] )
    {
    ...
    {
        damage( ch, victim, number_range( 1, ch->level ), sn, DAM_BASH, true );
        check_improve( ch, sn, true, 1 );
    }
    else
    {
...
void do_disarm( CHAR_DATA *ch, const char *argument )
{
    ...
    int                     ch_weapon = 0;
    int                     vict_weapon = 0;
    int                     ch_vict_weapon = 0;
    int                     sn = -1;

    if ( ( sn = skill_lookup( "disarm" ) ) == -1 )
    {
        log_error( "Can't find the \"%s\" skill in %s?", "disarm", __FUNCTION__ );
        return;
    }

    if ( ( chance = get_skill( ch, sn ) ) == 0 )
    {
        ch_printf( ch, "You don't know how to disarm opponents.\r\n" );
        return;
    }
    ...
    {
        ch_printf( ch, "You must wield a weapon to disarm.\r\n" );
        return;
    }
    ...
    if ( ( obj = get_eq_char( victim, WEAR_WIELD ) ) == NULL )
    {
        ch_printf( ch, "Your opponent is not wielding a weapon.\r\n" );
        return;
    }
