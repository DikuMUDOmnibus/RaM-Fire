/*
 * RAM $Id: interp.c 67 2009-01-05 00:39:32Z quixadhal $
 */
...
void interpret( CHAR_DATA *ch, const char *argument )
{
    ...
    char                    logline[MAX_INPUT_LENGTH] = "\0\0\0\0\0\0\0";
    int                     cmd = 0;
    int                     trust = 0;
    bool                    found = false;
    ...
    {
        ch_printf( ch, "You're totally frozen!\r\n" );
        return;
    }
    ...
    {
        log_auth( ch, "Caught %s doing %s", NAME( ch ), logline );
        wiz_printf( ch, NULL, WIZ_SECURE, 0, get_trust( ch ),
                    "Caught %s doing %s", NAME( ch ), logline );
    }

    if ( ch->desc != NULL && ch->desc->snoop_by != NULL )
    {
        desc_printf( ch->desc->snoop_by, "%% %s\r\n", logline );
    }
    ...
    if ( ch->position < cmd_table[cmd].position )
    {
        ...
        {
            case POS_DEAD:
                ch_printf( ch, "Lie still; you are DEAD.\r\n" );
                break;

            case POS_MORTAL:
            case POS_INCAP:
                ch_printf( ch, "You are hurt far too bad for that.\r\n" );
                break;

            case POS_STUNNED:
                ch_printf( ch, "You are too stunned to do that.\r\n" );
                break;
            ...
            case POS_RESTING:
                ch_printf( ch, "Nah... You feel too relaxed...\r\n" );
                break;

            case POS_SITTING:
                ch_printf( ch, "Better stand up first.\r\n" );
                break;

            case POS_FIGHTING:
                ch_printf( ch, "No way!  You are still fighting!\r\n" );
                break;
...
bool check_social( CHAR_DATA *ch, const char *command, const char *argument )
{
    ...
    {
        ch_printf( ch, "You are anti-social!\r\n" );
        return true;
    }
...
int number_argument( const char *argument, char *arg )
{
    ...
    int                     number = 0;
    char                    num[MAX_INPUT_LENGTH] = "\0\0\0\0\0\0\0";
    char                   *ndot = num;
...
void do_commands( CHAR_DATA *ch, const char *argument )
{
    ...
    for ( cmd = 0; cmd_table[cmd].name[0] != '\0'; cmd++ )
    {
        ...
        {
            ch_printf( ch, "%-12s", cmd_table[cmd].name );
            if ( ++col % 6 == 0 )
                ch_printf( ch, "\r\n" );
        }
    }
